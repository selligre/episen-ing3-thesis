...
CURRENT STATE: The idea is to produce a data pipeline (in Python) that scans a player's game data to establish a statistical profile in order to separate casual / expert / cheater players to highlight suspicious profiles to direct them towards a manual review. I have currently started to develop the pipeline but I am still making modifications to the data model and implementing the notion of LSTM and their consideration in the scoring algorithm.
...
\begin{itemize}
\item Aim Vectors: Time-to-Damage (\gls{TTD}), pre-aim delta, and spray pattern deviation
...
\item Tactical and Utility Metrics: Grenade/HE efficiency (Average \gls{HE} damage), Clutch performance variance, and overall Kill/Death (KD)
